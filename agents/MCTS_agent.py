...
def clean_graph(state, goal_spec, last_opened):
    ...
    for predicate in goal_spec:
        ...
        for x in elements[1:]:
            if x.isdigit():
                ...
                nodes_missing += [
                    node["id"] for node in state["nodes"] if node["class_name"] == x
                ]
    nodes_missing += [
        node["id"]
    ...
    for key in goal_spec:
        ...
        if elements[0] == "off":
            if id2node[int(elements[2])]["class_name"] in [
                "dishwasher",
                "kitchentable",
            ]:
                augmented_class_names += [
...
def get_plan(
    ...
    if (
        ...
        and last_action.split(" ")[0] == "[putin]"
        and remained_to_put == 0
    ...
    root_node = Node(
        id=(
            root_action,
            [init_vh_state, init_state, goal_spec, satisfied, unsatisfied, 0, []],
        ),
        num_visited=0,
        sum_value=0,
        is_expanded=False,
    ...
    next_root, plan, subgoals = mcts.run(
        curr_node, nb_steps, heuristic_dict, last_subgoal, opponent_subgoal
    )
    if verbose:
        print("plan", plan)
