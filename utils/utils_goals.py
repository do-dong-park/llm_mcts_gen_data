def convert_goal_spec(task_names, goal, state, exclude=[]):
    """
    ...
    for key_count in goal:
        ...
        for task_name in task_names:
            if task_name in ['setup_table', 'prepare_food', 'prepare_drink', 'prepare_snack', 'prepare_wash']:
                predicate = 'on_{}_{}'.format(elements[1], elements[3])
                goals[predicate] = count
            elif task_name in ['put_dishwasher', 'put_fridge', 'put_bathroom_cabinet', 'put_microwave', 'put_kitchencabinet']:
